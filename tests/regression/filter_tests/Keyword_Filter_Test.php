...
EOL;
		...
		// Keywords that should be matched at a minimum (Selected by human)
		private $expectedKeywords = array("half-life","gabe newell","valve","game development","half-life franchise","alex osborn");
    	...
    	public function testExtractionOfKeywords()
    	{
			...
			if(!isset($this->testDocument->keywords)||sizeof($this->testDocument->keywords)<=0)
			{
				$allKeywordsPresent = false;
			}
			
			$keyWordsString = '';
			foreach($this->testDocument->keywords as $resultKeyword)
			{
				$keyWordsString.=  $resultKeyword." ";
			}
			
			foreach($this->expectedKeywords as $keyword)
			{
				if(strpos($keyWordsString, $keyword) !== false) 
				{
					//echo 'Keyword is present';
				}
				else
				{
					$allKeywordsPresent = false;
